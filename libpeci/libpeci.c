                                 ...
                                 uint8_t* pPkgConfig, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pPkgConfig, u8ReadLen, cmd.pkg_config, u8ReadLen) !=
            PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
                                 ...
                                 int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pPCIData, 4, cmd.pci_config, 4) != PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
    }
...
/*-------------------------------------------------------------------------
 * This function provides sequential read access to the PCI configuration space
 *at the requested PCI configuration address with the provided peci file
 *descriptor.
 *------------------------------------------------------------------------*/
                                         ...
                                         uint8_t* cc)
{
    ...
    if ((u8Len > PECI_RDPCICFG_READ_LEN_MAX))
    {
        return PECI_CC_INVALID_REQ;
    }

    // 4-byte read for one RdPCIConfig() call
    iCount = (u8Len + 3) / 4;

    for (regIndex = 0; regIndex < iCount; regIndex++)
    {
        cmd.addr = target;
        cmd.bus = u8Bus;
        cmd.device = u8Device;
        ...
        *cc = cmd.cc;
        ...
        // Make sure we do not overflow the output buffer
        u8CopyCount = 4;
        if (((regIndex + 1) * 4) > u8Len)
        {
            u8CopyCount = u8Len - (regIndex * 4);
        }
        if (memcpy_s(&pPCIData[regIndex * 4], u8CopyCount, cmd.pci_config,
                     u8CopyCount) != PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
                                      ...
                                      uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pPCIReg, u8ReadLen, cmd.pci_config, u8ReadLen) !=
            PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
    ...
    uint8_t* pPCIData, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pPCIData, u8ReadLen, cmd.data, u8ReadLen) !=
            PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
    ...
    uint8_t u8ReadLen, uint8_t* pMmioData, int peci_fd, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    ...
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pMmioData, u8ReadLen, cmd.data, u8ReadLen) !=
            PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
                                     ...
                                     uint8_t* pData, uint8_t* cc)
{
    ...
    *cc = cmd.cc;
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pData, u8ReadLen, cmd.data, u8ReadLen) != PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
    }
                     ...
                     uint32_t respSize)
{
    ...
    if (memcpy_s(u8TxBuf, sizeof(u8TxBuf), pRawCmd, cmdSize) != PECI_CC_SUCCESS)
    {
        peci_Close(peci_fd);
        return PECI_CC_MEM_ERR;
    }
    ...
    if (ret == PECI_CC_SUCCESS)
    {
        if (memcpy_s(pRawResp, u8ReadLen, u8RxBuf, u8ReadLen) !=
            PECI_CC_SUCCESS)
        {
            ret = PECI_CC_MEM_ERR;
        }
